...
def get_table(soup, ind):
    ...
    if ind == 0:
        headings = ["Team", "W", "L", "SV", "G", "GS", "IP", "SO", "K/9", "BB/9",
                    "HR/9", "BABIP", "LOB%", "GB%", "HR/FB", "ERA", "FIP", "xFIP", "WAR"]
    else:
        headings = ["Season", "Team", "W", "L", "SV", "G", "GS", "IP", "SO", "K/9",
...
def team_pitching(start_season, end_season=None, league='all', ind=1):
    ...
    if start_season is None:
        raise ValueError(
            ...
            "Try team_pitching(season) or team_pitching(start_season, end_season)."
        )
...
def team_pitching_bref(team, start_season, end_season=None):
    ...
    if start_season is None:
        raise ValueError(
            "You need to provide at least one season to collect data for. " +
            "Try team_pitching_bref(season) or team_pitching_bref(start_season, end_season)."
        )
    if end_season is None:
        end_season = start_season
    ...
    for season in range(start_season, end_season+1):
        ...
        for row in rows:
            ...
            # Remove '*' and '#' from some names
            cols = [col.replace('*', '').replace('#', '') for col in cols]

            # Remove Team Totals and other rows
            cols = [col for col in cols if 'Totals' not in col and 'NL teams' not in col and 'AL teams' not in col]
    ...
    return data.reset_index(drop=True, inplace=True)  # Fixes index issue (Index was named 'W" for some reason)
