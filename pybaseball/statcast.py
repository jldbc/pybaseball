...
def sanitize_input(start_dt, end_dt):
    ...
    if start_dt is None and end_dt is None:
        ...
        print(
            "Warning: no date range supplied. " +
            "Returning yesterday's Statcast data. " +
            "For a different date range, try get_statcast(start_dt, end_dt)."
        )
...
def large_request(start_dt, end_dt, d1, d2, step, verbose):
    """
    Break start and end date into smaller increments, collecting all data in small
    chunks and appending all results to a common dataframe. end_dt is the date string
    for the final day of the query d1 and d2 are datetime objects for first and last
    day of query, for doing date math a third datetime object (d) will be used to
    increment over time for the several intermediate queries
    """
        ...
        if data.shape[0] > 1:
            ...
            while success == 0:
                ...
                if data.shape[0] > 1:
                    ...
                    error_counter += 1
                if error_counter > 2:
                    # This request is probably too large. Cut a day off of this request and
                    # make that its own separate request. For each, append to dataframe list.
                    # If successful, skip and print error message if failed
                    ...
                    if smaller_data_1.shape[0] > 1:
                        ...
                        print(
                            "Query unsuccessful for data from {} to {}. Skipping these dates.".format(start_dt, tmp_end)
                        )
...
def postprocessing(data, team):
    ...
    numeric_cols = ['release_speed', 'release_pos_x', 'release_pos_z', 'batter', 'pitcher', 'zone', 'hit_location',
                    ...
                    'outs_when_up', 'inning', 'hc_x', 'hc_y', 'fielder_2', 'vx0', 'vy0', 'vz0', 'ax', 'ay', 'az',
                    'sz_top', 'sz_bot', 'hit_distance_sc', 'launch_speed', 'launch_angle', 'effective_speed',
                    'release_spin_rate', 'release_extension', 'game_pk', 'pitcher.1', 'fielder_2.1', 'fielder_3',
                    ...
                    'babip_value', 'iso_value', 'launch_speed_angle', 'at_bat_number', 'pitch_number', 'home_score',
                    'away_score', 'bat_score', 'fld_score', 'post_away_score', 'post_home_score', 'post_bat_score',
                    'post_fld_score']

    data[numeric_cols] = data[numeric_cols].astype(float)

    # Convert date col to datetime data type and sort so that this returns in an
    # Order that makes sense (by date and game)
    ...
    valid_teams = ['MIN', 'PHI', 'BAL', 'NYY', 'LAD', 'OAK', 'SEA', 'TB', 'MIL', 'MIA',
                   ...
                   'PIT', 'NYM', 'CLE', 'CWS', 'STL', 'WSH', 'SF', 'SD', 'BOS', 'ARI',
                   'ANA', 'WAS']
...
def statcast(start_dt=None, end_dt=None, team=None, verbose=True):
    ...
    INPUTS:
    start_dt    : str 'YYYY-MM-DD'                  : the first date for which you want statcast data
    end_dt      : str 'YYYY-MM-DD'                  : the last date for which you want statcast data
    team        : optional str (defaults to None)   : city abbreviation of the team you want data for (e.g. SEA or BOS)
