...
def get_season(start_dt: str, end_dt: str) -> str:
    start_date = start_dt.split("-")
    start_year = int(start_date[0])
    end_date = end_dt.split("-")
    end_year = int(end_date[0])

    if end_year == start_year:
        return str(start_year) + "%7C"
    else:
        seasons = str(end_year) + "%7C"
        for i in range(end_year - start_year):
            seasons = seasons + str(end_year - (i+1)) + "%7C"
        return seasons