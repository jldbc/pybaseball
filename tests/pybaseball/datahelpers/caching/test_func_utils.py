...
class TestCachingFuncUtils:
    def test_get_value_hash_str(self) -> None:
        test_str = "TESTING"

        assert caching.get_value_hash(test_str) == f"'{test_str}'"

    def test_get_value_hash_str_no_designators(self) -> None:
        test_str = "TESTING"

        assert caching.get_value_hash(test_str, include_designators=False) == test_str

    def test_get_value_hash_str_with_bad_chars(self) -> None:
        test_str = "TESTING/"

        assert caching.get_value_hash(test_str) == str(test_str.__hash__())

    def test_get_value_hash_hashable(self) -> None:
        test_float = 3.14159

        assert caching.get_value_hash(test_float) == str(test_float.__hash__())

    def test_get_value_hash_none(self) -> None:
        test_value = None

        assert caching.get_value_hash(test_value) == 'None'

    def test_get_value_hash_list(self) -> None:
        test_value = ['a', 'b']

        assert caching.get_value_hash(test_value) == "['a', 'b']"

    def test_get_value_hash_list_no_designators(self) -> None:
        test_value = ['a', 'b']

        assert caching.get_value_hash(test_value, include_designators=False) == "'a', 'b'"
    ...
    def test_get_value_hash_dict(self) -> None:
        ...
        assert caching.get_value_hash(test_value) == "{a='foo', b='bar'}"

    def test_get_value_hash_dict_no_designators(self) -> None:
        test_value = {'a': 'foo', 'b': 'bar'}

        assert caching.get_value_hash(test_value, include_designators=False) == "a='foo', b='bar'"
    ...
    def test_get_value_hash_non_hashable(self) -> None:
        ...
        with pytest.raises(ValueError):
            print(caching.get_value_hash(NonHashable()))

    def test_get_func_name_function(self) -> None:
        def test_func() -> None:
            pass

        assert caching.get_func_name(test_func) == "test_func"
    ...
    def test_get_func_name_class_method(self) -> None:
        ...
        assert caching.get_func_name(test_class().test_func) == "test_class.test_func"

    def test_get_func_hash_no_params(self) -> None:
        def test_func(*args: Any, **kwargs: Any) -> None:
            pass

        assert caching.get_func_hash(test_func, (), {}) == "test_func()"
    ...
    def test_get_func_hash_with_args(self) -> None:
        ...
        assert caching.get_func_hash(test_func, ('a', 1), {}) == expected_hash
    ...
    def test_get_func_hash_with_kwargs(self) -> None:
        ...
        assert caching.get_func_hash(test_func, (), {'val1': 1.5, 'val2': 'b'}) == expected_hash
    ...
    def test_get_func_hash_with_args_and_kwargs(self) -> None:
        ...
        assert caching.get_func_hash(test_func, ('a', 1), {'val1': 1.5, 'val2': 'b'}) == expected_hash
    ...
    def test_get_func_hash_long_path_windows(self) -> None:
        ...
        long_kwargs = {f"val{str(x)}": x for x in range(caching.MAX_ARGS_KEY_LENGTH)}

        assert caching.get_func_hash(test_func, ('a', 1), long_kwargs) == expected_hash
